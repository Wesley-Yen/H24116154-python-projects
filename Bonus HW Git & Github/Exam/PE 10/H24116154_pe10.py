...
# 初始化障礙物
obstacles = []
buffer_zone = snake + [[snake_y, snake_x + 1], [snake_y, snake_x + 2]]  # 添加緩衝區

while len(obstacles) < int(sh * sw * 0.05):
    if random.choice([True, False]):
        y = random.randint(1, sh - 2)
        x = random.randint(1, sw - 7)
        new_obstacle = [[y, x + i] for i in range(5)]
    else:
        y = random.randint(1, sh - 7)
        x = random.randint(1, sw - 2)
        new_obstacle = [[y + i, x] for i in range(5)]
    
    # 確保新障礙物不在緩衝區內
    if not any(cell in buffer_zone for cell in new_obstacle):
...
while True:
    ...
    if next_key == -1:
        next_key = key  # 沒有按鍵時保持原方向
    elif next_key == 32:  # 按空格鍵暫停/恢復
        paused = not paused
        next_key = key  # 暫停時不改變方向
